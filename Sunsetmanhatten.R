### Manhatten sunset plot
library(methylKit)
library(tidyverse)
library(qqman)
qq(myDiff$pvalue)

#methylkitworkspace<-("/home/sren/Documents/Master_thesis/Crappyfish/methylkit_19samples_5X_210203_myDiff.RData")
...
myDiff_t<-as_tibble(myDiff) #%>% gsub("NW.*", "other",chr)
#%>% filter(grepl("NC_027", chr, fixed = T))
rm(myDiff_MN)
rm(meth.filtered)
...
ggplot(SMP, aes(x=BPcum, y=(SMvalue))) +
  ...
  scale_color_manual(values = rep(c("orange", "skyblue"), 600 )) +
